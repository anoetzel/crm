# Пояснения к выполненному диплому

Реализованы все дополнительные задания.

Т.к. при нажатии на кнопу "Удалить" у клиента в списке модальное окно загружается очень быстро, то прелоадер я поставила на состояние :active, иначе он никогда не будет показываться.

Карточку клиента я сделала не совсем идентичной форме для редактирования клиента, иначе, получается, каждый, у кого есть ссылка, может отредактировать клиента, что не очень хорошо. Идентификатор клиента в таблице является ссылкой на его карточку.

Сначала я сверстала форму добавления клиента тегом form, но отказалась от этой идеи потом, т.к. нажатие на кнопу "Добавить контакт" воспринимается как отправка формы по клику. event.preventDefault не помогал. Кнопку добавления надо было, наверное, располагать под кнопкой. Но я в итоге отказалась от этого тега, заменив его на div.
